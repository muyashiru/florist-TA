...
const startIndex = content.indexOf(htmlStart.replace(/\\n/g, '\\r\\n'));
const endIndex = content.indexOf(htmlEnd.replace(/\\n/g, '\\r\\n'));

let finalStartIndex = startIndex;
let finalEndIndex = endIndex;

if (finalStartIndex === -1) finalStartIndex = content.indexOf(htmlStart);
if (finalEndIndex === -1) finalEndIndex = content.indexOf(htmlEnd);

if (finalStartIndex === -1 || finalEndIndex === -1) {
    console.error("HTML boundary not found!");
    console.log("Start:", finalStartIndex, "End:", finalEndIndex);
    process.exit(1);
}
...
content = content.substring(0, finalStartIndex) + newHtml + content.substring(finalEndIndex);

const oldFetchScenarios = /async function fetchScenarios\(\) \{[\s\S]*?const data = await res\.json\(\);[\s\S]*?const sel = document\.getElementById\('scenarioSelect'\);[\s\S]*?sel\.innerHTML = '<option value="">-- Pilih Skenario Tersimpan --<\/option><option value="NEW">✨ -- Mulai Skenario Baru \(Kosong\) --<\/option>';[\s\S]*?if\(data\.scenarios\) \{[\s\S]*?data\.scenarios\.forEach\(s => \{[\s\S]*?sel\.innerHTML \+= '<option value="'\+s\.id\+'">'\+s\.name\+'<\/option>';[\s\S]*?\}\);[\s\S]*?\}[\s\S]*?\}/;
