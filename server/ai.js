...
async function searchProductsInDB(userMessage, chatHistory = []) {
    try {
        ...
        // 4. Fallback: Jika tidak ditemukan, kembalikan 3 produk default agar AI tidak pernah melihat katalog kosong!
        const [fallbackRows] = await db.query('SELECT sku, name, category, size, price, image_url FROM products WHERE available = 1 LIMIT 3');
        return fallbackRows;
    } catch (err) {
...
export async function askQwenAI(senderNumber, userMessage) {
...
5. ALUR PEMESANAN KETAT (5 TAHAP SOP TOKO):
   - TAHAP 1 (Cek Jadwal & Urgensi): Jika pelanggan mau pesan (datang dari web atau WA), TANYAKAN DULU untuk tanggal & jam berapa pesanan ingin dikirim/diterima?
     * Jika pesan URGENT (< 5 jam dari sekarang): Langsung alihkan ke admin dengan pesan santun + kode [HANDOFF].
     * Jika memesan Fresh Bouquet (BFP_, BFL_, BFS_): Wajib minimal H-3 pengiriman. Jika kurang dari H-3, tawarkan Artificial Bouquet yang tahan lama atau alihkan ke admin.
     * Jika memesan Artificial Bouquet (BAP_, BAS_, BAM_, BAL_, BAXL_, BAHS_): Wajib minimal H-1 pengiriman.
   - TAHAP 2 (Kirim Form): JIKA DAN HANYA JIKA tanggal pengantaran sudah valid sesuai aturan H-3 (Fresh) atau H-1 (Artificial), kirimkan TEMPLATE FORM PEMESANAN berikut:
     "Attention !!
   ...
   - TAHAP 4 (Bukti Transfer -> Handoff): Jika pelanggan sudah mengirimkan foto/screenshot bukti pembayaran (atau mengatakan sudah transfer/bayar), ucapkan terima kasih dan WAJIB langsung akhiri dengan kode: [HANDOFF] agar admin manusia melakukan verifikasi pembayaran.
...
6. BATASAN AI LAINNYA:
   - Jika pelanggan punya foto referensi desain dari luar (Pinterest/TikTok/IG/custom rumit), kirim bukti transfer, minta revisi foto produksi, tanya ready-stock di toko fisik, atau eksplisit minta bicara dengan admin manusia → BERIKAN PESAN PENDEK dan akhiri jawabanmu dengan kode rahasia: [HANDOFF].
...
------------------------------------`;
    ...
    try {
        ...
        return {
            reply: aiReply || 'Maaf Kak, sistem AI kami sedang sibuk 🌸',
            products: relevantProducts
        };
    } catch (error) {
        console.error('❌ Gagal menghubungi OmniRoute:', error.message);
        return {
            reply: `❌ [Gagal Koneksi ke OmniRoute]: ${error.message}. (Pastikan aplikasi OmniRoute sudah menyala di port 20128!)`,
            products: []
        };
    }
}
